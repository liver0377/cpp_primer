...
StrBlobPtr &StrBlobPtr::operator--() {
    check(curr - 1, "decrease past start of StrBlobPtr");
    --curr;
    return *this;
}
...
StrBlobPtr &StrBlobPtr::operator+(int x) {
    check(curr + x - 1, "increment past end of StrBlobPtr");
    StrBlobPtr t = *this;
    for (int i = 0; i < x; i++)
        ++t;
    return t;
...
StrBlobPtr &StrBlobPtr::operator-(int x) {
    check(curr - x, "decrease past start of StrBlobPtr");
    StrBlobPtr t = *this;
    for (int i = 0; i < x; i++)
        --t;
    return t;
